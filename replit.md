...
- **/gencoupon (Owner)** — 5-step wizard in `promo.js` (`ctx.session.adminGenCoupon`): discount (`pct 10`/`flat 500`), scope (`all`/`cat A,B`/`prod <search>`), total uses (`unlimited` ok), per-user limit, expiry days (`never` ok). Auto code `MGS-XXXXXX` via `PromoService.generateCoupon()` (5 retries on dup key)
