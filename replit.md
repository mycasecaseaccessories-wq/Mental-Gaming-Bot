...
- **Free Giveaway** (`commands/accountGiveaway.js`): give things away free — now covers **ALL premium account types (single/shared/invite) AND regular shop `Product`s**. `AccountGiveaway.kind` enum `account`/`shop`: account giveaways set `productId` (ref AccountProduct), shop giveaways set `shopProductId` (ref Product); two partial unique indexes (one per kind) so **multiple giveaways can run at once, one per product** (`isActive` non-unique; `getActives()` returns all active, populates both refs). `gaMeta(ga)` normalises either kind → `{doc,kind,id,emoji,title,sub,price,accountType?,isMulti?}`; `gaStock(ga)` gives live availability (shop = `stockCount`/-1 unlimited/isActive; account single = `countAvailable`, multi = `countAvailableSlots`). Restrictions all toggleable per giveaway: maxClaims 0=unlimited, endAt, minAccountAgeDays via `utils/accountAge`, requirePurchase = Order status 'Success', requireChannelId getChatMember check. `AccountGiveawayClaim` (unique giveawayId+telegramId — one claim per user **per giveaway**, claim-record-first, rollback on stock failure; adds `slotId` ref AccountSlot + `couponId` ref Promo for the new paths). **Claim = 3 paths**: (1) **shop** → mint personal 100%-off `GIFT` coupon via `PromoService.generateCoupon` (scopeType product, scopeProducts:[shopProductId], restrictedToUserId, perUserLimit 1, source reward) → user redeems in `/shop`; (2) **multi (shared/invite)** → `claimSlots`+`AccountSlot.create`, `releaseSlots` rollback on failure; (3) **single** → `claimOne` pricePaid 0. Auto-deactivates that giveaway on quota/stock exhaustion + owner notify (`autoEndAndNotify`). Admin: `/accadmin` → 🎁 Free Giveaway → **list of giveaways** (`accga_view:<id>`) + ➕ add (`accga_new`→**kind picker** `accga_newkind:account|shop`→`accga_pick:<productId>` / `accga_pickshop:<productId>` creates); per-giveaway detail actions all keyed by id (`accga_toggle/max/days/age/purch/chan/chanoff/del/announce:<id>`, `accga_chansel:<gaId>:<chatId>`, `accga_repick:<id>`→`accga_setprod:<gaId>:<id>` (account) / `accga_setshop:<gaId>:<id>` (shop)); reply-targeted text wizard `session.accGaWiz` stores `gaId`. Announce via `broadcastToUsers` (claim button targets `accga_free:<id>`) + announcement channel deep link `?start=freebie` (body uses `gaMeta`). User: `/accounts` 🎁 banner/button (banner shows if any active giveaway of either kind) or `/freebie` → `accga_free` entry shows single detail if 1 active, else a picker list (`accga_free:<id>`); claim `accga_claim:<id>`.
